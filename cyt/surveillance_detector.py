...
class SurveillanceDetector:
    ...
    def enhance_with_fingerprints(self, session) -> List[SuspiciousDevice]:
        """
        Re-analyze surveillance patterns using SSID-pool fingerprints.

        Devices sharing a Fingerprint (i.e. same physical device with randomized MACs)
        have their appearances merged, producing a single SuspiciousDevice with a
        boosted persistence score.
        """
        from cyt.models import Device, Appearance, Fingerprint
        import json as _json

        fingerprints = session.query(Fingerprint).all()
        merged_devices: List[SuspiciousDevice] = []

        # Collect fingerprinted device IDs so we skip them in the normal pass
        fingerprinted_macs = set()

        for fp in fingerprints:
            linked = session.query(Device).filter_by(fingerprint_id=fp.id).all()
            if not linked:
                continue

            all_appearances: List[DeviceAppearance] = []
            for dev in linked:
                fingerprinted_macs.add(dev.mac)
                rows = session.query(Appearance).filter_by(device_id=dev.id).all()
                for row in rows:
                    ssids = []
                    if row.ssids_json:
                        try:
                            ssids = _json.loads(row.ssids_json)
                        except (ValueError, TypeError):
                            pass
                    loc = f"{row.lat:.4f},{row.lon:.4f}" if row.lat and row.lon else "unknown"
                    all_appearances.append(DeviceAppearance(
                        mac=dev.mac,
                        timestamp=row.timestamp.timestamp(),
                        location_id=loc,
                        ssids_probed=ssids,
                        signal_strength=row.signal_dbm,
                    ))

            if len(all_appearances) < self.thresholds['min_appearances']:
                continue

            score, reasons = self._calculate_persistence_score(all_appearances)

            # Boost: device uses MAC randomization → more suspicious
            reasons.insert(0, f"MAC randomization detected — {len(linked)} MACs share SSID fingerprint")
            score = min(score + 0.25, 1.0)

            timestamps = [a.timestamp for a in all_appearances]
            merged_devices.append(SuspiciousDevice(
                mac=fp.canonical_mac,
                ...
                locations_seen=list(set(a.location_id for a in all_appearances)),
            ))

        # Also include non-fingerprinted devices from the normal analysis
        normal = self.analyze_surveillance_patterns()
        for dev in normal:
            if dev.mac not in fingerprinted_macs:
                merged_devices.append(dev)

        merged_devices.sort(key=lambda d: d.persistence_score, reverse=True)
